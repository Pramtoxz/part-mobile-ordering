...
import ahm.parts.ordering.ui.home.dealer.salesmanvisit.realisasivisit.adapter.CoordinatorSelectedAdapter
import ahm.parts.ordering.ui.home.dealer.salesmanvisit.realisasivisit.adapter.KodeDealerSelectedAdapter
import ahm.parts.ordering.ui.home.dealer.salesmanvisit.realisasivisit.adapter.SalesmanSelectedAdapter
...
class RealisasiVisitActivity : BaseActivity<RealisasiVisitViewModel>(), View.OnClickListener {
    ...
    private fun initUI() {
        setToolbar(getString(R.string.lbl_title_realisasi_visit), true)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == Constants.REQUEST.KODE_DEALER && resultCode == Constants.RESULT.KODE_DEALER) {
            ...
            etPilihDealer.hint = getString(R.string.hint_add_dealer)

            Log.e("dealerSelecteds",dealerSelecteds.getString())
            ...
            etKoordinatorSalesman.hint = getString(R.string.hint_add_koordinator)
    ...
    override fun onClick(v: View?) {
        when (v!!) {
                    ...
                    strDateEnd.isEmpty() -> {
                        ...
                        when (user.id_role) {
                            ...
                            Constants.ROLE.MANAGER -> {
                                ...
                                goTo<PlanActualRealisasiCoordinatorManagerActivity> {
                                    putExtra(Constants.BUNDLE.PARAM,paramPlanActual.getString())
                                }
                            }
                            else -> {
                                ...
                                goTo<PlanActualRealisasiKoordinatorActivity> {
                                    putExtra(Constants.BUNDLE.PARAM,paramPlanActual.getString())
                                }
                            }
                        }
            ...
            etKoordinatorSalesman -> {
                goTo<KoordinatorSalesmanRealisasiActivity>(requestCode = Constants.REQUEST.KOORDINATOR_SALESMAN) {
            ...
            etSalesman -> {
                if (user.id_role != Constants.ROLE.SALESMAN) {
                        goTo<SalesmanRealisasiVisitActivity>(requestCode = Constants.REQUEST.SALESMAN_SELECT) {
            ...
            etPilihDealer -> {
                if (user.id_role == Constants.ROLE.SALESMAN) {
                    goTo<KodeDealerRealisasiVisit>(requestCode = Constants.REQUEST.KODE_DEALER) {
                        //salesmanSelected = Salesman()
                        putExtra(Constants.BUNDLE.JSON, salesmanSelecteds.getString())
