...
class PlanActualDetailRealisasiVisitTabActivity : BaseActivity<PlanActualRealiasiViewModel>(), View.OnClickListener {

    var planActuals = ArrayList<RealisasiDetail>()
    ...
    private fun initUI(){
        ...
        val data = extra(Constants.BUNDLE.JSON).getObject<Data>()
        planActuals.addAll(data.detail)

        for (i in 0 until planActuals.size){
            adapter.addFragment(PlanActualDetailRealisasiFragment.newInstance(i, planActuals[i],this@PlanActualDetailRealisasiVisitTabActivity), "")
        }
