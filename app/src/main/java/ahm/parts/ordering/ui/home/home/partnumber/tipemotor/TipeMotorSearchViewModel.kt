...
import ahm.parts.ordering.data.model.home.dashboard.partnumber.TypeMotor
...
class TipeMotorSearchViewModel @Inject constructor(private val context : Application) : BaseViewModel(){

    val typeMotorSearchs = MutableLiveData<TypeMotor>()

    fun hitTypeMotor(strSearch : String) {

        apiResponse.postValue(ApiResponse().responseLoading())

        compositeDisposable.add(
            apiService.getPartMotor(strSearch)
                ...
                .subscribe({
                    ...
                    if (apiStatus == Constants.REMOTE.API_STATUS_SUCCESS) {
                        ...
                        val kelompokBarangData = responseJson.obj(Constants.REMOTE.OBJ_DATA).obj(Constants.REMOTE.OBJ_DATA).getObject<TypeMotor>()
                        typeMotorSearchs.value = kelompokBarangData
