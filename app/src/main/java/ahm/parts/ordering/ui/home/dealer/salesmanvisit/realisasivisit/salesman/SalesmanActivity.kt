package ahm.parts.ordering.ui.home.dealer.salesmanvisit.realisasivisit.salesman
...
class SalesmanActivity : BaseActivity<RealisasiVisitViewModel>(), ClickPrevention {
    ...
    var salesmansSelected = ArrayList<Salesman>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        salesmansSelected = extra(Constants.BUNDLE.JSON).toList<Salesman>() as ArrayList<Salesman>
    ...
    private fun observeData(){
        ...
        viewModel.salesmans.observe(this, Observer {
             ...
             */
            if (salesmans.size == salesmansSelected.size) {
                for (d in salesmans.indices) {
                    val itemMember = salesmansSelected[d]
                ...
                for (d in salesmans.indices) {
                    val itemCard = salesmans[d]
                    for (z in salesmansSelected.indices) {
                        val itemMember = salesmansSelected[z]
                        if (itemCard.id == itemMember.id) {
                            itemCard.selectedSales = true
                        }
                    }
