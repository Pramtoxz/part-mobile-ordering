...
class SliderCarouselCompetitorAdapter(private val context: Activity) : PagerAdapter(), CardAdapter {
    private val cardViews: MutableList<CardView>
    ...
    fun addCardItem(items: List<String>) {
        ...
        for (i in cslItems.indices) {
            cardViews.add(CardView(context))
        }
    }
